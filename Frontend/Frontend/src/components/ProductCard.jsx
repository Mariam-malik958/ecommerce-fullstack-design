    ...
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow group">
      ...
      <div className="p-3">
        <Link to={`/products/${product.id}`}>
          <p className="text-xs text-gray-400 mb-1">From USD {product.price}</p>
          <h3 className="text-sm font-medium text-gray-800 hover:text-blue-600 transition line-clamp-2 mb-2">
            {product.name}
          </h3>
        </Link>
        ...
        <button
          ...
          className="w-full mt-1 text-blue-600 border border-blue-600 text-xs py-1.5 rounded hover:bg-blue-600 hover:text-white transition font-medium"
        >
          Add to cart
        </button>
      </div>
