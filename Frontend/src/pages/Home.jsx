...
import { FaFacebookF, FaTwitter, FaLinkedinIn, FaInstagram, FaYoutube, FaShoppingCart, FaBars, FaTimes } from 'react-icons/fa';
...
const services = [
  { 
    ...
    fallback: "https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=400&q=80",
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="11" cy="11" r="8" stroke="#0D6EFD" strokeWidth="1.5" fill="none"/>
        <path d="M21 21L17.5 17.5" stroke="#0D6EFD" strokeWidth="1.5" strokeLinecap="round"/>
      </svg>
    )
  },
  { 
    ...
    img: "/images/11.png", 
    fallback: "https://images.unsplash.com/photo-1558769132-cb1aea458c5e?auto=format&fit=crop&w=400&q=80",
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M6 9V7.5C6 6.67 6.67 6 7.5 6H16.5C17.33 6 18 6.67 18 7.5V9M6 9V19.5C6 20.33 6.67 21 7.5 21H16.5C17.33 21 18 20.33 18 19.5V9M6 9H18" stroke="#0D6EFD" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M10 12V18" stroke="#0D6EFD" strokeWidth="1.5" strokeLinecap="round"/>
        <path d="M14 12V18" stroke="#0D6EFD" strokeWidth="1.5" strokeLinecap="round"/>
      </svg>
    )
  },
  { 
    label: "Fast, reliable shipping by ocean or air", 
    img: "/images/image 106.png", 
    fallback: "https://images.unsplash.com/photo-1578575437130-527eed3abbec?auto=format&fit=crop&w=400&q=80",
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M3 11L12 3L21 11V18C21 19.1 20.1 20 19 20H5C3.9 20 3 19.1 3 18V11Z" stroke="#0D6EFD" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M12 3V8H20" stroke="#0D6EFD" strokeWidth="1.5" strokeLinecap="round"/>
        <path d="M9 16H15" stroke="#0D6EFD" strokeWidth="1.5" strokeLinecap="round"/>
      </svg>
    )
  },
  { 
    ...
    fallback: "https://images.unsplash.com/photo-1586953208448-b95a79798f07?auto=format&fit=crop&w=400&q=80",
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="12" cy="12" r="9" stroke="#0D6EFD" strokeWidth="1.5" fill="none"/>
        <path d="M12 6V12L16 14.5" stroke="#0D6EFD" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    )
  },
];
...
// Footer Different Countries/Languages List
...
export default function Home() {
  ...
  const [footerLangOpen, setFooterLangOpen] = useState(false); // Footer dropdown state
  ...
  const handleMenuClick = (item) => showNotification(`Clicked: ${item}`);

  const handleCategoryClick = (index, category) => {
    setActiveCategory(index);
    showNotification(`Selected: ${category}`);
    setTimeout(() => navigate("/products", { state: { category } }), 300);
  };
  ...
  const handleAddToCart = (e, product) => {
    e.stopPropagation();
    addToCart({ id: product.id, name: product.name, price: product.price, image: product.image || product.img });
  ...
  return (
    <div className="bg-[#F7FAFC] min-h-screen text-[#333]" style={{ fontFamily: "'DM Sans', Arial, sans-serif" }}>
      ...
      <nav className="bg-white border-b border-[#E5E7EB] py-3.5 sticky top-0 z-50 shadow-sm">
        <div className="max-w-[1200px] mx-auto px-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="w-full md:flex-1 md:max-w-[600px] flex border-2 border-[#0D6EFD] rounded-lg overflow-hidden bg-white">
            <input type="text" placeholder="Search..." className="flex-1 border-none px-3 py-2 outline-none text-sm" />
            ...
            <button className="bg-[#0D6EFD] text-white px-5 font-semibold text-sm hover:bg-[#0b5ed7] transition-colors" onClick={() => showNotification("🔍 Searching...")}>
              Search
            </button>
          </div>
          ...
          <div className={`${mobileMenuOpen ? "flex" : "hidden"} md:flex flex-row gap-2 justify-around md:justify-end items-center w-full md:w-auto mt-2 md:mt-0`}>
            <button className="flex flex-col items-center gap-1 p-2 rounded-lg text-gray-600 hover:bg-[#E8F1FF] hover:text-[#0D6EFD] transition-all">
        ...
        <div className="max-w-[1200px] mx-auto px-4 flex justify-between items-center py-3" style={{ flexWrap: "nowrap" }}>
          <div className="flex items-center gap-6" style={{ flexShrink: 1, minWidth: 0, overflow: "hidden" }}>
            ...
            <div className="flex gap-5 text-sm text-gray-700">
              {["Hot offers", "Gift boxes", "Projects", "Menu item", "Help ▾"].map((item) => (
                <span key={item} className="cursor-pointer hover:text-[#0D6EFD] whitespace-nowrap" onClick={() => handleMenuClick(item)}>
                  {item}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center gap-3" style={{ flexShrink: 0 }}>
            ...
            <div className="tm-dd-wrap" onClick={(e) => e.stopPropagation()}>
              <div
                className="tm-dd-trigger"
                onClick={() => { setLangOpen(!langOpen); setShipOpen(false); }}
              >
              ...
              {langOpen && (
                <div className="tm-dd-menu">
                  {LANGUAGES.map((l) => (
                    <div
                      key={l.text}
                      className="tm-dd-item"
                      onClick={() => {
                        setSelectedLang(l);
                        setLangOpen(false);
                        showNotification(`💱 Changed to ${l.text}`);
                      }}
                    >
                      {l.flag} {l.text}
                    </div>
            ...
            <div className="tm-dd-wrap" onClick={(e) => e.stopPropagation()}>
              <div
                className="tm-dd-trigger"
                onClick={() => { setShipOpen(!shipOpen); setLangOpen(false); }}
              >
                <span>Ship to</span>
                  ...
                  {COUNTRIES.map((c) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        setShipOpen(false);
                        showNotification(`📍 Shipping to ${c.name}`);
                      }}
                    >
      ...
      <div className="max-w-[1200px] mx-auto px-4">
        ...
        {/* HERO GRID */}
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr_220px] gap-4 mt-5">
          <div className="bg-white border border-[#E5E7EB] rounded-lg p-2.5 hidden lg:block">
            {categories.map((item, i) => (
              <div key={item} className={`sidebar-item ${activeCategory === i ? "active" : ""}`} onClick={() => handleCategoryClick(i, item)}>
          ...
          <div className="rounded-lg p-8 sm:p-10 flex items-center min-h-[280px] sm:min-h-[320px] bg-cover bg-center" style={{ backgroundImage: "url('/images/Banner-board.png')" }}>
            <div className="relative z-10">
              <p className="text-sm sm:text-base text-gray-600 font-medium">Latest trending</p>
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-3">
            <div className="bg-[#E8F1FF] p-4 rounded-lg flex-1">
              ...
              <button className="w-full bg-[#0D6EFD] text-white font-semibold text-sm py-2 rounded-md hover:bg-[#0b5ed7] mt-2">Join now</button>
              <button className="w-full bg-white text-[#0D6EFD] border border-[#0D6EFD] font-semibold text-sm py-2 rounded-md hover:bg-[#E8F1FF] mt-2">Log in</button>
            </div>
        ...
        <div className="mt-5 bg-white border border-[#E5E7EB] rounded-lg grid grid-cols-1 lg:grid-cols-[260px_1fr] overflow-hidden">
          <div className="p-6 flex flex-col justify-center bg-cover bg-center min-h-[150px] lg:min-h-[250px]" style={{ backgroundImage: "url('/images/Group 969.png')" }}>
            ...
            <button className="bg-white text-black text-xs font-bold px-4 py-2 rounded-md w-fit hover:bg-gray-100 shadow">Source now</button>
          </div>
      ...
      <footer className="bg-white border-t border-gray-200 pt-10 mt-12">
          ...
          {["About", "Partnership", "Information", "For users"].map((title) => (
            <div key={title}>
              ...
              <div className="flex flex-col gap-2 text-sm text-gray-600 footer-links">
                ...
                {/* NEW: App Store and Google Play buttons integrated for users section */}
                {title === "For users" && (
        ...
        <div className="border-t border-gray-200 mt-10 py-5 bg-[#F7FAFC]">
          <div className="max-w-[1200px] mx-auto px-4 flex flex-col sm:flex-row justify-between items-center text-xs text-gray-500 gap-2">
            ...
            {/* Right-Aligned Clickable Dropdown Menu with Flags */}
